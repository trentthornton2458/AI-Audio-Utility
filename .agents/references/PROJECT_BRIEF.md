...
## Distilled Brief
Add a local, zero-cost 'Humanizer' stage to AI-Audio-Utility's existing vocal processing chain (BS-RoFormer separation → resemble-enhance denoise → Pedalboard DSP → resemble-enhance polish → QA-gated blend) so Suno v5.5 vocals sound less robotic. The stage applies pyrubberband-driven micro-pitch drift and automatic residual breath/noise blend-back after the existing enhance/QA-blend step, gated by new warn-only QA metrics, with a bundled-reference blind A/B UI and a visibly stubbed RVC/Seed-VC control for future voice conversion.
