...
class ABCompareView(QWidget):
    ...
    def load_reference(
        ...
        if file_path_or_key and Path(file_path_or_key).is_file():
            loaded_path = Path(file_path_or_key)
            self._reference_path = loaded_path
            self._active_reference_key = loaded_path.stem
        else:
            meta = vocal_metadata or self._vocal_metadata
            key, selected_path = select_reference_stem(vocal_metadata=meta)
            self._reference_path = selected_path
            self._active_reference_key = key if selected_path else None
