...
class WaveformCanvas(QWidget):
    ...
    playPauseRequested = Signal()  # Emits to toggle play/pause state
    ...
    def keyPressEvent(self, event) -> None:
        ...
        if event.key() == Qt.Key.Key_Left:
            ...
            self.seekRequested.emit(duration_ms)
            event.accept()
        else:
            super().keyPressEvent(event)
...
class WaveformPlayerWidget(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self._canvas.playPauseRequested.connect(self.on_play_clicked)
        self._canvas.setToolTip(
            "Interactive Waveform Display\n"
            "• Click/drag to seek\n"
            "• When focused, use Space to Play/Pause, Home/End to seek to start/end,\n"
            "  Left/Right arrow keys (5s), or PageUp/PageDown (15s)"
        )
        ...
        self._canvas.setAccessibleDescription(
            ...
            "Space to play/pause, Home/End to jump to start/end, and arrows/page-keys to seek."
        )
