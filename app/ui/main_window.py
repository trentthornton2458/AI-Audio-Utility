...
class FileLoadPanel(QFrame):
    ...
    def _init_ui(self) -> None:
        ...
        self._browse_button.setToolTip(
            "Select a Suno audio file (.wav or .mp3) from your computer"
        )
        self._browse_button.setAccessibleName("Browse Audio File")
        self._browse_button.setAccessibleDescription(
            "Opens a file dialog to choose a Suno audio track in WAV or MP3 format."
        )
...
class StemSeparationPanel(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self._extract_button.setToolTip(
            "Separate the loaded audio track into separate vocal and instrumental stems. This button is disabled until an audio track is successfully ingested."
        )
        self._extract_button.setAccessibleName("Extract Stems")
        self._extract_button.setAccessibleDescription(
            "Splits the loaded Suno audio track into separate vocal and instrumental stems using the BS-RoFormer neural model."
        )
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self) -> None:
        ...
        self._render_button.setToolTip(
            "Apply current vocal and instrumental settings, then render and master the final track. This button is disabled until an audio track is successfully ingested."
        )
        self._render_button.setAccessibleName("Render and Master Track")
        self._render_button.setAccessibleDescription(
            "Processes the stems with the current neural cleanup and DSP settings, then mixes and masters the final track."
        )
