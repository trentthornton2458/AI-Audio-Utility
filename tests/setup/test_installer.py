...
from app.setup.installer import (check_cuda_dll, check_pyside6_plugins,
                                 get_system_ram_gb, is_frozen, run_diagnostics)
...
def test_check_pyside6_plugins_success():
    ...
    with (
        patch.object(QCoreApplication, "libraryPaths", return_value=mock_paths),
...
def test_get_system_ram_gb_linux_success():
    with (
        patch("sys.platform", "linux"),
        patch("os.sysconf", side_effect=[4096, 4194304], create=True),
    ):  # 4096 * 4194304 = 16 GB
        assert get_system_ram_gb() == 16.0
...
def test_run_diagnostics():
    with (
        patch("app.setup.installer.check_pyside6_plugins", return_value=True),
