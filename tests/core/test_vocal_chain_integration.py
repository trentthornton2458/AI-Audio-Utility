...
from app.core import separation, humanizer

# Import mocks from test_full_pipeline
from tests.test_full_pipeline import MockSeparator, mock_denoise, mock_enhance

def mock_apply_pitch_drift_intensity_aware(audio: np.ndarray, sample_rate: int, intensity: float) -> np.ndarray:
    if intensity == 0.0:
        return audio
...
def test_vocal_chain_pipeline_humanizer_and_qa_metrics(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    def mock_enhance_clipping(current: np.ndarray, current_sr: int, device: str) -> tuple[np.ndarray, int]:
    ...
    monkeypatch.setattr(humanizer, "apply_pitch_drift", mock_apply_pitch_drift_intensity_aware)
    ...
    job_off = RenderJob(input_path=input_file, preset=preset_off, cache_manager=cache_mgr)
    ...
    assert len(humanizer_files) >= 2, "Expected distinct cached outputs for different humanizer intensities"

    # Verification 2: Measurably different outputs
    assert not np.allclose(audio_off, audio_on), "Output with humanizer=0 should differ from output with humanizer>0"

    # Verification 3: Breath-blend/humanizer stage does not introduce clipping
    assert np.max(np.abs(audio_on)) <= 1.0, "Audio output should not clip after humanizer stage"
    ...
    assert "clipping" in reasons, "Expected the Milestone 3 QA gate to flag clipping from the mock enhance"
