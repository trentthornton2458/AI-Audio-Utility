...
def test_get_gemini_api_key_returns_stored_value():
    with patch(
        "app.models.gemini_settings.keyring.get_password", return_value="my-secret-key"
    ) as mock_get:
        result = gemini_settings.get_gemini_api_key()

    assert result == "my-secret-key"
    mock_get.assert_called_once_with(
        gemini_settings.SERVICE_NAME, gemini_settings.USERNAME
    )
...
def test_set_gemini_api_key_saves_to_credential_store():
    ...
    mock_set.assert_called_once_with(
        gemini_settings.SERVICE_NAME, gemini_settings.USERNAME, "new-key"
    )
